...
let countries_data2 = countries_data.sort(function (a, b) {
...
function mostSpokenLanguages(number) {
	...
	countries_data2.forEach((country) => {
	...
	for (let i = 0; i < number; i++) {
	  ...
	  returnArray.push(allSpokenlanguages[i]);
	}
	return returnArray;
  }

let populationArray = [{ name: "World", population: 0 }];
let languageArray = [];

languageArray =  mostSpokenLanguages(10);

for (let country of countries_data2) {
  populationArray.push({ name: country.name, population: country.population });
}

let sum = 0;
populationArray.forEach((element) => (sum += element.population));

populationArray[0].population = sum;
let width = 500;

let popBtn = document.querySelector('.population-btn');
let lang = document.querySelector('.languages-btn');
...
for (let i = 0; i < 11; i++) {
  document.querySelector(
    ".graphs .name"
  ).innerHTML += `<p>${populationArray[i].name}</p>`;

  document.querySelector(
    ".graphs .graph-bar"
  ).innerHTML += `<div class="bar bar${i}"></div>`;

  width = (populationArray[i].population * 500)/7758589152;
  document.querySelector(`div.bar${i}`).style.width = `${width}px`

  document.querySelector(
    ".graphs .population"
  ).innerHTML += `<p>${populationArray[i].population}</p>`;
}
